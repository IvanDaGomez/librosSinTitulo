...
export class MessagesModel {
    ...
    static async sendMessage(data) {
        try {
            ...
            await pool.query(`INSERT INTO messages (id, user_id, message, conversation_id, created_in, read)
         VALUES ($1, $2, $3, $4, $5, $6)`, [newMessage._id, newMessage.userId, newMessage.message, newMessage.conversationId, newMessage.createdIn, newMessage.read]);
            return true;
        }
        catch (err) {
    ...
    static async updateMessage(id, data) {
        try {
            await pool.query('UPDATE messages SET message = $1, read = $2 WHERE id = $3 RETURNING *', [data.message, data.read, id]);
            return true;
        }
        catch (err) {
