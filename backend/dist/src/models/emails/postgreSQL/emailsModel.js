...
export class EmailsModel {
    ...
    static async createEmail(data) {
        try {
            ...
            const result = await pool.query('INSERT INTO emails (email) VALUES ($1) RETURNING *', [data.email]);
    ...
    static async updateEmail(id, data) {
        try {
            ...
            const result = await pool.query('UPDATE emails SET email = $1 WHERE id = $2 RETURNING *', [data.email, id]);
            if (result.rowCount === 0) {
