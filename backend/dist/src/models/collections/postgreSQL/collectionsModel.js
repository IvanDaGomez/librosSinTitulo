...
class CollectionsModel {
    ...
    static async createCollection(data) {
        try {
            const { rows } = await pool.query(`INSERT INTO collections (foto, nombre, descripcion, user_id, saga)
         VALUES ($1, $2, $3, $4, $5) RETURNING *`, [data.foto, data.nombre, data.descripcion, data.userId, data.saga]);
    ...
    static async updateCollection(id, data) {
        try {
            const { rows } = await pool.query(`UPDATE collections 
         SET foto = $1, nombre = $2, descripcion = $3, saga = $4 
         WHERE id = $5 RETURNING *`, [data.foto, data.nombre, data.descripcion, data.saga, id]);
            return rows.length ? collectionObject(rows[0]) : null;
        }
        catch (err) {
