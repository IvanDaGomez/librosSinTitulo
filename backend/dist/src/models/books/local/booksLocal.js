...
class BooksModel {
    static async getAllBooks() {
        ...
        return books.map(book => bookObject(book, true));
    }
    ...
    static async getBookByQuery(query, l, books = []) {
        ...
        const filterdBooks = booksWithScores
            .sort((a, b) => b.score - a.score)
            .map(item => item.book)
            .filter(item => item.disponibilidad === 'Disponible')
            .slice(0, l);
        const bookToReturn = filterdBooks.map(item => bookObject(item, false));
    ...
    static async getBooksByQueryWithFilters(query) {
        ...
        books = books.filter(book => {
            return Object.keys(query.where).some(filter => book[filter] ===
                query.where[filter]);
        });
        // Perform search based on the query
        books = (await this.getBookByQuery(query.query, query.limit, books));
        if (books === undefined || !books) {
            return [];
        }
        const bookToReturn = books
    ...
    static async forYouPage(userKeyInfo, sampleSize = 100) {
        ...
        const selectedBooks = randomIndexes
            ...
            .filter(element => element !== undefined && element.disponibilidad === 'Disponible');
