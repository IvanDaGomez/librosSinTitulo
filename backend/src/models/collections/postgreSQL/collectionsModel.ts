...
class CollectionsModel {
  ...
  static async updateCollection (id: ID, data: Partial<CollectionObjectType>): Promise<CollectionObjectType> {
    try{
          ...
          const result = await executeSingleResultQuery(
            pool,
            () => pool.query(
              `UPDATE collections SET ${updateString} WHERE ID = $${keys.length + 1} RETURNING *;`,
              [...values, id]
            ),
  ...
  static async getCollectionByQuery (query: string, l: number = 1000, collections: CollectionObjectType[] = []): Promise<CollectionObjectType[]> {
    ...
    const filteredCollections = collectionsWithScores.sort((a, b) => b.score - a.score).map(item => item.collection)
  ...
  static async getCollectionSaga (bookId: ID, userId: ID): Promise<CollectionObjectType> {
    const collection = await executeSingleResultQuery(
      pool,
      () => pool.query('SELECT * FROM collections WHERE userId = $1 AND librosIds @> $2 AND saga = true;', [userId, [bookId]]),
      'Failed to fetch collection from PostgreSQL'
    )
    if (collection) {
      return collection
  ...
  static async forYouPageCollections (userKeyInfo: any, sampleSize: number): Promise<CollectionObjectType[]> {
    const collections = await executeQuery(
      pool,
      () => pool.query(`SELECT * FROM collections WHERE userId != $1 ORDER BY RANDOM() LIMIT $2;`, [userKeyInfo.userId, sampleSize]),
      'Failed to fetch collections from PostgreSQL'
    )
