...
export function filterBooksByFilters (books: BookObjectType[], preparedFilters: any): BookObjectType[] {
  return books.filter(book => {
    ...
    return filterKeys.every((filterKey: string) => {
      if (filterKey === 'minPrecio' || filterKey === 'maxPrecio') {
        const price = book.precio as number
        const minPrecio = preparedFilters['minPrecio'] as number | undefined
        const maxPrecio = preparedFilters['maxPrecio'] as number | undefined
        if ((minPrecio !== undefined && price < minPrecio) || (maxPrecio !== undefined && price > maxPrecio)) {
          return false
        }
        ...
        const filterValues = preparedFilters[filterKey] as string[] | undefined
        if (filterValues && !bookKeyInfo.some(key => 
          filterValues.some((value: string) => key.toLowerCase() === value.toLowerCase())
        )) {
          return false
        }
      }
