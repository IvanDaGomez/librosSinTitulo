  ...
  static async createConversation (data: Partial<ConversationObjectType>): Promise<ConversationObjectType> {
    ...
    const newConversation = await executeSingleResultQuery(
      ...
      () => pool.query(
        ...
        [fullConversation.id, fullConversation.users, fullConversation.createdIn, fullConversation.lastMessage]
      ),
  ...
  static async updateConversation (id: ID, data: Partial<ConversationObjectType>): Promise<ConversationObjectType> {
    try {
      ...
      const updateString = keys
        ...
        .join(', ');
      const query = `UPDATE conversations SET ${updateString} WHERE id = $${keys.length + 1} RETURNING *;`
      ...
      throw new DatabaseError(`Error updating conversation with ID ${id}`, error);
    }
  }
}
