async function sendOpenAIRequest(imageUrl: string, filepath: string): Promise<object | null> {

  
  // Create the prompt
  if (!filepath) return null
  console.log('imageUrl', imageUrl)
  const GPTContext = 'Eres un asistente de IA que extrae información de portadas de libros. Tu tarea es analizar la imagen y extraer información relevante sobre el libro. Proporciona la información en formato JSON.'
  const prompt = `
